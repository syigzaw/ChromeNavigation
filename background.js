...
function navigateChrome(text) {
    ...
    if (words.indexOf('go') >= 0 && words.indexOf('to') == words.indexOf('go') + 1) {
        ...
        switchByTabName(words);
    }
}

function switchByTabName(words){
    chrome.tabs.getAllInWindow(null, function (tabs) {
        for (var i = 0; i < tabs.length; i++) {
            var name = tabs[i].title.split(' ');
            name = name[0];
            if (name.toLowerCase().contains(words[1])) {
                chrome.tabs.update(tabs[i].id, { active: true });
                break;
            }
            // The above implementation could expand more easily
            // to take care of more words to switch tab by
            /*var title = tabs[i].title.toLowerCase();
            if (title.indexOf(words[1]) == 0) {
                chrome.tabs.update(tabs[i].id, { active: true });
                break;
            }*/
        }
